...
(letrec-syntax
  ...
  (let* (;;==========================================================================
    ...
    (define (expand-or exp)
      (match exp
        ((or) #f) ; (or) => #f, Larceny r6399, bug #647 #648
        ((or e) (expand e))
        ((or e es ___)
         `(let ((x ,(expand e)))
            (if x x ,(expand `(,or ,@es)))))))
                           ...
                           (let* ((original-env *usage-env*)
                                  ...
                                  (rhs-expanded
                                   (fluid-let ((*phase* (+ 1 *phase*))
                                     ...
                                     (map expand rhs)))
                                  (macros (map (lambda (e) (eval-core e)) rhs-expanded)))
    ...
    (define (current-builds imported-libraries)
      (map (lambda (lib-entry)
             (ex:library-build (ex:lookup-library (car lib-entry) #f))) ;MOSH: pass recompile flag
           imported-libraries))
    ...
    (define (run-r6rs-sequence forms)
      (with-toplevel-parameters
       (lambda ()
         (for-each (lambda (exp) (eval-core exp))
...
;;MOSH: cache
    ...
    (define (normalize exps)
      (define (error)
        (let ((newline "")) ; MOSH: do not produce #\linefeed for gosh..
          (syntax-violation
           'expand-file
           (string-append
