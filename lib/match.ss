...
(library (match)
  ...
  (import
   (only (rnrs) - < length let* >= reverse cons vector-length = + vector-ref define-syntax syntax-rules _ ... vector? define if let pair? let-syntax begin car cdr lambda error quote null? equal? and or list? quasiquote unquote unquote-splicing => cond else)
   (rnrs mutable-pairs))
...
(define-syntax match-two
  (syntax-rules (___ quote quasiquote ? $ = and or not set! get!)
